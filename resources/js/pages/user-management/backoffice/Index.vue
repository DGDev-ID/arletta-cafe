...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Manage Backoffice', href: '/user-management/backoffice' },
];
...
const filterSearch = () => {
    router.get('/user-management/backoffice', { search: searchQuery.value }, { preserveState: true });
};

const revokeAccess = (userId: number) => {
    if (confirm('Apakah Anda yakin ingin mencabut akses Backoffice dari user ini?')) {
        router.delete(`/user-management/backoffice/${userId}/revoke`);
    }
};
...
watch(assignSearch, (val) => {
    ...
    debounceTimer = setTimeout(async () => {
        ...
        try {
            const { data } = await axios.get('/user-management/backoffice/search-users', { params: { q: val } });
...
const assignUser = (userId: number) => {
    router.post('/user-management/backoffice/assign', { user_id: userId }, {
        onSuccess: () => {
            assignSearch.value = '';
            assignResults.value = [];
        },
                        ...
                        <tbody>
                            ...
                            <tr v-if="data.data.length === 0">
                                <td colspan="4" class="px-6 py-10 text-center text-muted-foreground">
                                    Belum ada user dengan role Backoffice.
                                </td>
                            </tr>
                        </tbody>
                    </table>
